...
class ViewsController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        // dd($total_debt->amountloaned);
        // dd($total_debt->amountloaned);
        //log user out if not active
        $loan= loan::where('user_id', Auth::user()->id)->where('status', 'processed')->first();
    ...
    {
        // $pg = DB::table('loans')->where('status', 'pending')->where('user_id', Auth::user()->id)->orderBy('id', 'desc')
        //             ->first();
        //             dd($pg);
        return view('user.transactions')
            ->with(array(
                ...
                'withdrawals' => Withdrawal::where('user', Auth::user()->id)->orderBy('id', 'desc')
                    ...
                    'amountborrowed' => loan::where('user_id', Auth::user()->id)->orderBy('id', 'desc')
                    ->get(),
    ...
    public function deposits()
    {
        ...
        //sum total deposited
        $total_deposited = DB::table('deposits')->select(DB::raw("SUM(amount) as count"))->where('user', Auth::user()->id)->where('status', 'Processed')->get();
        $total_loaned = DB::table('loans')->select(DB::raw("SUM(amountloaned) as count"))->where('user_id', Auth::user()->id)->where('status', 'Processed')->get();
        $loan = Loan::where('user_id', Auth::user()->id)->where('status', 'Processed')->where('paid', 'no')->first();
        // $loaner = Loan::where('user_id', Auth::user()->id)->where('status', 'pending')->where('paid', 'no')->first();
    ...
    { 
        $loan= loan::where('user_id', Auth::user()->id)->where('status', 'Processed')->orderBy('id', 'desc')->first();
        // dd($loan->staus);
        if($loan->paid == 'no'){
            return redirect()->route('dashboard',)

            ->with('message','you have to pay your oustanding loan to be able to use this feature');
        }
        else{
